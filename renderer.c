				       ...
				       float zDistance)
{
	...
	if (camera.projection == CAMERA_PERSPECTIVE)
	{
		...
		matProj = MatrixPerspective(
			camera.fovy * DEG2RAD,
			((double)screenWidth / (double)screenHeight), 0.01f,
			zDistance);
	}
	...
	Vector3 nearPoint = Vector3Unproject(
		(Vector3){deviceCoords.x, deviceCoords.y, 0.0f}, matProj,
		matView);
	Vector3 farPoint = Vector3Unproject(
		(Vector3){deviceCoords.x, deviceCoords.y, 1.0f}, matProj,
		matView);

	// Calculate normalized direction vector
	Vector3 direction =
		Vector3Normalize(Vector3Subtract(farPoint, nearPoint));
						 ...
						 void *userData)
{
	...
	for (int i = 0; i < text.length; ++i)
	{
		...
		if (fontToUse.glyphs[index].advanceX != 0)
			...
			lineTextWidth += (fontToUse.recs[index].width +
					  fontToUse.glyphs[index].offsetX);
	}
...
void Clay_Raylib_Render(Clay_RenderCommandArray renderCommands, Font *fonts)
{
	for (int j = 0; j < renderCommands.length; j++)
	{
		...
		case CLAY_RENDER_COMMAND_TYPE_TEXT: {
			// Raylib uses standard C strings so isn't compatible
			// with cheap slices, we need to clone the string to
			// append null terminator
			Clay_TextRenderData *textData =
				&renderCommand->renderData.text;
			char *cloned = (char *)malloc(
				textData->stringContents.length + 1);
			memcpy(cloned, textData->stringContents.chars,
			       textData->stringContents.length);
			cloned[textData->stringContents.length] = '\0';
			Font fontToUse = fonts[textData->fontId];
			DrawTextEx(fontToUse, cloned,
		...
		case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
			Texture2D imageTexture =
				*(Texture2D *)renderCommand->renderData.image
					 .imageData;
			Clay_Color tintColor =
				renderCommand->renderData.image.backgroundColor;
			if (tintColor.r == 0 && tintColor.g == 0 &&
			    tintColor.b == 0 && tintColor.a == 0)
			{
				tintColor = (Clay_Color){255, 255, 255, 255};
			}
			DrawTextureEx(
				imageTexture,
				(Vector2){boundingBox.x, boundingBox.y}, 0,
		...
		case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
			...
			{
				DrawRectangle(boundingBox.x, boundingBox.y,
					      boundingBox.width,
					      boundingBox.height,
					      CLAY_COLOR_TO_RAYLIB_COLOR(
						      config->backgroundColor));
			...
			if (config->cornerRadius.topLeft > 0)
			{
				DrawRing((Vector2){roundf(boundingBox.x +
						   ...
						   roundf(boundingBox.y +
							  config->cornerRadius
								  .topLeft)},
					 roundf(config->cornerRadius.topLeft -
						config->width.top),
			...
			if (config->cornerRadius.topRight > 0)
			{
				DrawRing((Vector2){roundf(boundingBox.x +
						   ...
						   roundf(boundingBox.y +
							  config->cornerRadius
								  .topRight)},
					 roundf(config->cornerRadius.topRight -
						config->width.top),
		...
		case CLAY_RENDER_COMMAND_TYPE_CUSTOM: {
			...
			case CUSTOM_LAYOUT_ELEMENT_TYPE_3D_MODEL: {
				Clay_BoundingBox rootBox =
					renderCommands.internalArray[0]
						.boundingBox;
				...
				DrawModel(
					...
					customElement->customData.model.scale *
						scaleValue,
